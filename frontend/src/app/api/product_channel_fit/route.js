...
export async function GET(req) {
  try {
    ...
    const targetUrl = new URL("/api/product_channel_fit", API_BASE_URL);

    for (const key of ["year", "dimension", "top_n"]) {
      const val = searchParams.get(key);
      if (val) targetUrl.searchParams.set(key, val);
    }
    // Forward multi-year list parameters
    const yearListValues = searchParams.getAll("year_list");
    for (const y of yearListValues) {
