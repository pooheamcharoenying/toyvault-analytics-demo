...
export async function GET(req) {
  try {
    ...
    const targetUrl = new URL("/api/recommendations", API_BASE_URL);
    if (searchParams.get("context")) targetUrl.searchParams.set("context", searchParams.get("context"));
    if (searchParams.get("name")) targetUrl.searchParams.set("name", searchParams.get("name"));
