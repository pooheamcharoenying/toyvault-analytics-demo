...
export async function GET(req) {
  try {
    ...
    const targetUrl = new URL("/api/period_comparison", API_BASE_URL);
    for (const key of [
      "period_a_year", "period_a_quarter", "period_a_month",
      "period_b_year", "period_b_quarter", "period_b_month",
    ]) {
      if (searchParams.get(key)) targetUrl.searchParams.set(key, searchParams.get(key));
    }
