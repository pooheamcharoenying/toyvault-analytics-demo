...
export const maxDuration = 120; // agent turns can run a few tool calls
...
// Stateless one-shot chat: forwards the question to the backend agent.
export async function POST(req) {
  try {
    const body = await req.json();
    const target = new URL("/api/assistant/chat", API_BASE_URL);
    const r = await axios.post(target.toString(), body, { auth: _AUTH, timeout: 115000 });
