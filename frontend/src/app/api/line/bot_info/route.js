...
// Public identity of the LINE Official Account (name, basic id, QR add url, avatar).
export async function GET(req) {
  try {
    const url = new URL("/api/line/bot_info", API_BASE_URL).toString();
    const token = sessionToken(req);
    const headers = {};
    if (token) headers["X-Session-Token"] = token;
    const r = await axios({ method: "get", url, headers, auth: _AUTH, timeout: 120000 });
