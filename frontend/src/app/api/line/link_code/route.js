...
// Mint a one-time code that links the logged-in user's account to their LINE.
export async function POST(req) {
  try {
    const url = new URL("/api/line/link_code", API_BASE_URL).toString();
    const token = sessionToken(req);
    const headers = {};
    if (token) headers["X-Session-Token"] = token;
    const r = await axios({ method: "post", url, headers, auth: _AUTH, timeout: 120000 });
