...
export async function GET(req) {
  try {
    ...
    const dimension = searchParams.get("dimension") || "brand";
    const year = searchParams.get("year") || String(new Date().getFullYear());
    const topN = searchParams.get("top_n") || "10";
    const targetUrl = new URL("/api/concentration_summary", API_BASE_URL);
    targetUrl.searchParams.set("dimension", dimension);
